...
for child in root.findall('.//PhoneBook//uri'):
    ...
    for subchild in subroot.findall('contacts'):
        ...
        #croot = ctree.getroot()
        croot= ET.fromstring(response.content)
        sheetmaxrow1=sheetmaxrow
        sheetmaxrow2=sheetmaxrow
...
for child in root.findall('./Team/uri'):
    ...
    teamurlresponse=GET(teamurl,username,password)
    ...
    #subroot = subtree.getroot()
    subroot= ET.fromstring(teamurlresponse.content)
    ...
    #sub1root = sub1tree.getroot()
    sub1root= ET.fromstring(response.content)
    ...
    sub2root= ET.fromstring(response.content)
    ...
    sub3root= ET.fromstring(response.content)
    ...
    sub4root= ET.fromstring(response.content)
    ...
    sub5root= ET.fromstring(response.content)
    ...
    sub6root= ET.fromstring(response.content)
...
# Clear config.txt file
deleteconfig='n'
print('Delete config.txt yes/no?')
answer=input()
if answer=='y':
    deleteconfig='y'
elif answer == 'yes':
...
print('Task completed. Please review Finesse.XLS doc')
